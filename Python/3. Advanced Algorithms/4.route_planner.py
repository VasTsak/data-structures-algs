...
he algorithm you write will be responsible for generating a path like the one passed into show_map above. In fact, when called with the same map, start and goal, as above you algorithm should produce the path [5, 16, 37, 12, 34]
"""
...
#  create a map with the given roads and interactions
map_40 = map_graph()
...
class PriorityQueue:
    ...
    def __init__(self):
        # it will store a list of lists where the first element is the latest node of the path
        # and the second element is a list of the path
        self.queue = []
    ...
    def dequeue(self):
        ...
        for i in range(len(self.queue)):
            if self.queue[i].f < self.queue[min_index].f: # retieve the total cost from the current node
                min_index = i
...
def distance_between_nodes(parent, child):

    # Eucledian distance
    return ((parent[0] - child[0])**2 + (parent[1] - child[1])**2)**0.5


def shortest_path(map_graph, start, goal):
    """
    In this function, the A* search alorithm is implemented to find the
    shortest path for reaching the destination.

    Args:
        map_graph: graph data structure illustrating the map
        start: starting point
        goal: destination point

    Returns:
        path: a list containing all the cities to be visited in the right order
    """

    current_node = Node([], start)
    current_node.f = current_node.h = distance_between_nodes(map_graph.intersections[start], map_graph.intersections[goal])
    ...
    while current_node.position != goal:
        # set the path for the new nodes
        new_path = current_node.path.copy()
        new_path.append(current_node.position)
        # Initialize the distance for this current_node with infinity
        for option in map_graph.roads[current_node.position]:
            # assign the new node to the option
            new_node = Node(new_path, option)
            # calculate costs for the new nodes
            new_node.g = current_node.g + distance_between_nodes(map_graph.intersections[current_node.position], map_graph.intersections[option])
            new_node.h = distance_between_nodes(map_graph.intersections[option], map_graph.intersections[goal])
            new_node.f = new_node.g + new_node.h
            # add the new node to the current visited paths
            visited.enqueue(new_node)

        # remove current node from the paths to be explored
        try:
            visited.remove(current_node)
        except:
            pass
        current_node = visited.dequeue()
